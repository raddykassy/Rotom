...
def post_details():
    ...
    if request.method == 'POST':
        ...
        #入力データ取得
        data = request.form
        description = data["comment_li"] #場所ごとのコメント
        ...
        con = sqlite3.connect('Rotom.db')
        cur = con.cursor()
        cur.execute("""INSERT INTO plans (user_id, title, description, url) VALUES (?,?,?,?)""", (user_id[0], title, description, url))
        con.commit()
        ...
        for n  in range(len(session["place_names"])):
            cur.execute("INSERT INTO plan_places(plan_id, place_id, place_name, number, description, place_review, booking_url, price) VALUES(?,?,?,?,?,?,?,?)", (plan_id[0], place_id[n], place_names[n], n+1, description[n], place_review[n], booking_url[n], price[n]))
