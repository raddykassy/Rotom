...
def post_details():
    ...
    if request.method == 'POST':
        ...
        #場所ごとにplan_placesに格納
        for n  in range(len(session["place_names"])):
            cur.execute("INSERT INTO plan_places(plan_id, place_id, place_name, number, description, place_review, booking_url, price) VALUES(?,?,?,?,?,?,?,?)", (plan_id[0], place_id[n], place_names[n], n+1))
