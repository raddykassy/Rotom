...
def plan_content(username, post_id):
    ...
    place_info = cur.execute("SELECT * FROM plan_places WHERE plan_id = ?", (post_id,))
    plan_info = cur.execute("SELECT * FROM plans WHERE id=?", (post_id,))
    print(place_info, plan_info)
