...
def plan_content(username, post_id):
    ...
    place_info = list(cur.execute("SELECT * FROM plan_places WHERE plan_id = ?", (post_id,)).fetchall)
    plan_info = list(cur.execute("SELECT * FROM plans WHERE id=?", (post_id,)).fetchall)
    print(place_info, type(place_info))

    return render_template('content.html', plan_info = plan_info, username = username, place_info = place_info)