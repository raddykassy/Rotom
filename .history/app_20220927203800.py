...
def post():
    ...
    if request.method == 'POST':
        ...
        #セッション情報に追加
        #user_id, plan-title, plan-description, url
        #place_id, place_name, number, description, place_review, booking_url, price
...
def post_details():
    ...
    if request.method == 'POST':
        #plan-details.dbに保存
        rating = request.form.get("place-details-rating")
        price = request.form.get("place-details-price")
        booking_url = request.form.get("booking-url")
        place_details_description = request.form.get("place-details-description")

        data = request.form["data"].json()
        print(data[0])
        print("正常にpostできています")
        print(data[0]["rating_li"])
        print(data[0]["price_li"])
        print(data[0]["url_li"])

        print(type(data))
        

        return "正常にpostできています"
